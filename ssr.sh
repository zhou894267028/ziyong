...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/zhou894267028/ziyong/zhou-Linux-SSR/bbr.sh; then
...
Set_config_password(){
	...
	read -e -p "(默认: wenziju.com):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="wenziju.com"
